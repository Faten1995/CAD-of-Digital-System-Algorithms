        ...
        while True:
            ...
            for _ in range(len(self.graph.vertices)):
                ...
                #     D_values[y.id] += 2 * (c_yb) - 2 * (c_ya)
                    #print("c_yb", c_yb)
                    #print("c_ya", c_ya)
                    # print("D_values[y.id]", D_values[y.id])

                #print(str([int(a.id),int(b.id)]) + "A D value = " + str(D_values[a.id]) + " B D value = " + str(D_values[b.id]) + " Cab = " + str(c_ab) + " Maxgain = " + str(max_gain))
